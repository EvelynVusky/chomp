...
let check (vdecls, fdecls) = 
  ...
  (* returns true if type is primitive and not void. else, returns false. *)
  ...
  let rec convert_expr scope e = match e with
    ...
    | BinLit b -> 
      ...
      | x when x >= 2 && x <= 4 -> (scope, (Bit, SNibbleLit(b)))
      | x when x >= 5 && x <= 8 -> (scope, (Bit, SByteLit(b)))
      | x when x >= 9 && x <= 16 -> (scope, (Bit, SWordLit(b)))
    ...
    | Binop (e1, op, e2) -> 
        ...
        let ty = match op with
          ...
          | Concat when both_bin -> (get_larger_bin ty1 ty2)
          | Cons -> (match ty2 with
    ...
    | Call (name, es) -> 
      ...
      (* let (p_ty, p_val) = List.hd args'
      (match name with 
      "print" -> match p_ty with 
                Bool -> (scope, (ret, (Bool.to_string p_val)))
                | Int -> (scope, (string_of_int p_val))
                | Char -> (scope, (ret, SCall(name, )))
                | Bit | Nibble | Byte | Word -> 
                | String -> (scope, (ret, p_val))
      | "println" ->
      | "toBit" -> raise (InvalidError ("TODO"))
      | "toNibble" -> raise (InvalidError ("TODO"))
      | "toByte" -> raise (InvalidError ("TODO"))
      | "toWord" -> raise (InvalidError ("TODO"))
      | "toChar" -> raise (InvalidError ("TODO"))
      | "toInt" -> raise (InvalidError ("TODO"))
      | "set" -> raise (InvalidError ("TODO"))
      | "flipBit" -> raise (InvalidError ("TODO"))
      | "getBit" -> raise (InvalidError ("TODO"))
      | _ -> (scope, (ret, SCall(name, args')))) *)
    | List es as lst -> 
      match es with
  ...
  let check_fdecls (scope, acc) fdecl = 
    ...
    let _ = (match typ with
        Void -> (match sstmt_list with
                SBlock(SReturn((ret_typ, _)) :: _) -> eq_type_err2 ret_typ Void 
                | _ -> true)
        | _ -> (match sstmt_list with
              SBlock(SReturn((ret_typ, _)) :: _) -> eq_type_err2 ret_typ typ
